...
# -----------------------
# 创建 candies-sb 管理脚本[cite: 1]
SB_PATH="/usr/local/bin/candies-sb"[cite: 1]
info "正在创建 candies-sb 管理面板: $SB_PATH"[cite: 1]

cat > "$SB_PATH" <<'SB_SCRIPT'[cite: 1]
#!/usr/bin/env bash
set -euo pipefail
...
action_uninstall() {
    ...
    rm -rf /etc/sing-box /var/log/sing-box* /usr/local/bin/candies-sb /usr/bin/candies-sb /usr/local/bin/sb /usr/bin/sb /root/node_names.txt 2>/dev/null || true[cite: 1]
    info "卸载完成"
}
...
chmod +x "$SB_PATH"[cite: 1]
ln -sf /usr/local/bin/candies-sb /usr/bin/candies-sb[cite: 1]
info "✅ 管理面板已创建,可输入 candies-sb 打开管理面板"[cite: 1]
